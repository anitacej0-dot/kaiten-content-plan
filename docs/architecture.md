...
1. **Новые идеи из данных** (`ideation.py`) — генерируются из спроса по кластерам
   ...
   Дедуп в два слоя: против **истории** (Excel) и против **опубликованного блога**
   (URL из Roistat — по латинским брендовым токенам и интенту, надёжно без ошибок
   транслитерации). Помечаются как **гипотеза**.
