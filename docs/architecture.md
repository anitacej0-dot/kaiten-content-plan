...
| `ideation.py` | генерация НОВЫХ тем из спроса кластеров + паттернов редакции (дедуп с историей) |
...
Пул тем собирается из нескольких источников (модуль `candidates.py`):

1. **Новые идеи из данных** (`ideation.py`) — генерируются из спроса по кластерам
   (Roistat) и продуктивных паттернов редакции («Как выбрать…», «Аналоги
   <реальный конкурент>», «<продукт> для <сегмента>», связка Build→Convert→Prove).
   Всё, что уже было в истории, отсеивается. Помечаются как **гипотеза**.
2. **Переносы** — незавершённые темы недавних месяцев истории.
3. **Protect по Roistat** — топ-страницы блога по трафику → обновление.
4. **Кейсы из истории** — незавершённые кейсы → Prove.
5. **Сигналы редакции** — `seo_signals.csv`, `editorial_ideas.csv`,
   `mandatory_publications.csv`, `product_priorities.md`, `sales_kam_signals.md`.

Чем больше заполнено сигналов редакции (п. 5), тем больше и точнее новые темы.
